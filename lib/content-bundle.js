...
function createPaths(root) {
  ...
  const mediaRoot = process.env.MEDIA_ROOT?.trim() || path.join(publicDir, 'media');

  return {
    root,
    dataDir,
    mediaImages: path.join(mediaRoot, 'images'),
    mediaVideos: path.join(mediaRoot, 'videos'),
    manifestPath: path.join(dataDir, 'media-manifest.json'),
